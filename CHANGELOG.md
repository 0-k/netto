...
- **Year-aware health Zusatzbeitrag**: The official average Zusatzbeitrag now
  ...
  `None`, meaning "use the year's official average"; set it explicitly to
  match a specific Krankenkasse. Previously a flat 2.5% default was applied
  to every year, overstating health contributions for 2018-2024
- **Forecast data for 2027-2032**: All data categories extended with estimates
  for planning purposes, generated by the new `scripts/generate_forecast.py`
  (documented assumptions: 2%/year tax bracket indexation with frozen top
  bracket, 3%/year contribution ceiling growth, pension/nursing rate paths
  from official projections; see `data/README.md`)
...
### Fixed
- **Pension deduction factors 2023/2024**: Corrected from 0.92/0.96 to 1.0 —
  the Jahressteuergesetz 2022 made pension contributions fully deductible from
  2023 (the library previously overstated taxes for 2023/2024 by roughly
  100-250 €/year for typical salaries)
- **2026 tax curve coefficients**: Filled in the missing polynomial `const`
  values from the Steuerfortentwicklungsgesetz; the exact-formula method
  (`calc_income_tax`) previously crashed for 2026
